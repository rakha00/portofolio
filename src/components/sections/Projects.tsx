...
const featuredProjects: Project[] = [
  ...
  {
    title: "The Suma Group",
    role: "Freelance Frontend Developer",
    description:
      "A sophisticated company profile website for a premier financial consulting firm, designed and built using Next.js 16 and Tailwind CSS v4.",
    tech: ["NextJs", "Javascript", "Tailwind"],
    images: [
      "/projects/sumafinance/sumafinance-1.png",
      "/projects/sumafinance/sumafinance-2.png",
      "/projects/sumafinance/sumafinance-3.png",
      "/projects/sumafinance/sumafinance-4.png",
    ],
    liveUrl: "https://thesumagroup.com/",
    features: [
      "Responsive Layout: Fully optimized interface ensuring a seamless experience across all devices.",
      "Modern UI/UX: Sleek, trustworthy, and engaging user interface powered by interactive motion elements.",
      "Business Email Integration: Professional mailing system for seamless client inquiries and consultations.",
    ],
  },
  {
    title: "Mie Newmind",
  ...
  return (
    ...
    >
        ...
        <div className="relative w-full h-full group/slider">
          <AnimatePresence mode="wait">
            <motion.img
              ...
              onError={(e) => {
                e.currentTarget.src =
                  "https://placehold.co/800x450/18181b/indigo?text=Project+Screenshot";
              }}
            />
          </AnimatePresence>
          ...
          {images.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 hover:bg-indigo-600 text-white opacity-0 group-hover/slider:opacity-100 transition-all duration-300 backdrop-blur-sm border border-white/10 z-20"
              >
              ...
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 hover:bg-indigo-600 text-white opacity-0 group-hover/slider:opacity-100 transition-all duration-300 backdrop-blur-sm border border-white/10 z-20"
              >
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                {images.map((_, idx) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentImage ? "bg-indigo-500 w-6" : "bg-white/50 w-1.5 hover:bg-white"}`}
                  />
                ))}
              </div>
            </>
      ...
      <div className="lg:col-span-5 flex flex-col justify-center text-left">
        <div>
          <span className="text-indigo-400 font-mono text-xs tracking-wider uppercase mb-2 block">
            {project.role}
          </span>
            ...
            {project.features && (
              <ul className="mt-4 space-y-2">
                {project.features.map((feature, i) => (
                  <li
                    key={i}
                    className="flex items-center text-zinc-500 text-xs md:text-sm"
                  >
        ...
        <div className="flex flex-wrap gap-2 mb-8 mt-6">
          {project.tech.map((tech) => (
            <span
              key={tech}
              className="px-3 py-1 bg-zinc-900 text-zinc-400 text-xs font-mono rounded-full border border-zinc-800 hover:border-indigo-500/50 hover:text-indigo-300 transition-colors cursor-default"
            >
              {tech}
            </span>
          ))}
...
function CompactProjectCard({
  ...
  const Icon = project.title.includes("Plant")
    ? Brain
    : project.title.includes("Skoolio")
      ? BookOpen
      : Database;
...
export function Projects() {
  return (
    <section
      id="projects"
      className="py-24 bg-[#09090b] relative border-t border-zinc-900"
    >
